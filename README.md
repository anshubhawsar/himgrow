# ✨ Himgrow - Evolving the Digital Soul

> A modern, sophisticated personal portfolio and company showcase built with cutting-edge web technologies. Experience elegant design, smooth animations, and professional presentation of expertise in optimization, software engineering, and emerging AI/ML technologies.

[![Vercel Status](https://img.shields.io/badge/Deployed_on-Vercel-000000?style=flat-square&logo=vercel)](https://himgrow.vercel.app)
[![License](https://img.shields.io/badge/License-MIT-blue?style=flat-square)](LICENSE)
[![React](https://img.shields.io/badge/React-19.2.0-61DAFB?style=flat-square&logo=react)](https://reactjs.org)
[![Vite](https://img.shields.io/badge/Vite-7.3.1-646CFF?style=flat-square&logo=vite)](https://vitejs.dev)

---

## 🚀 Live Demo

**[Visit Himgrow →](https://himgrow.vercel.app)**

---

## 💡 Features

- 🎨 **Modern Design System** - Beautiful gradient themes with light/dark mode
- ⌨️ **Animated Typing Header** - Dynamic text typing animation like a terminal interface
- 📱 **Fully Responsive** - Seamless experience on desktop, tablet, and mobile
- ✨ **Smooth Animations** - Reveal animations, hover effects, and transitions
- 🌙 **Theme Switching** - Sun/Sunset transitions between light and dark modes
- 📋 **Contact Form** - Professional contact section with email & phone details
- 🔧 **Optimized Performance** - ~225KB JS, ~57KB CSS (gzipped)

---

## 🛠 Tech Stack

### Frontend
- **React 19.2.0** - UI framework
- **Vite 7.3.1** - Lightning-fast build tool
- **Tailwind CSS 4.2.1** - Utility-first CSS framework
- **Lucide React** - Beautiful icon library

### Build & Deployment
- **PostCSS** - CSS transformation
- **Autoprefixer** - CSS vendor prefixes
- **Vercel** - Production deployment

### Development
- **ESLint** - Code quality
- **Node.js** - Runtime environment

---

## 📋 Project Sections

### 1. **Hero Section**
- Animated typing text that cycles through inspiring quotes
- Call-to-action buttons linking to Contact section
- Floating background orbs and theme transition effects

### 2. **About Me** 
Technical expertise in:
- **Core Expertise**: Mathematical optimization, Linear Programming, Operations Research
- **Systems Engineering**: Computer architecture, competitive programming, embedded systems
- **Current Focus**: AI/ML technologies and machine learning algorithms

### 3. **Featured Projects**
- AI Security Agent - Autonomous security system with reinforcement learning
- Thermal Process Simulator - Advanced thermal gradient analysis for welding
- Robotics Kinematics Solver - Multi-axis robotic motion calculations

### 4. **Technical Skills**
- **Languages**: C++, Python, Arduino C
- **Specialized Knowledge**: Operations Research, Linear Programming, Robotics, RL
- **Competencies**: Algorithm Optimization, System Architecture, Memory Management

### 5. **Contact Section**
- Direct email and phone contact options
- Professional contact form
- Easy accessibility for collaboration inquiries

---

## 🏃 Quick Start

### Prerequisites
- Node.js (v16+)
- npm or yarn

### Installation

```bash
# Clone the repository
git clone https://github.com/anshubhawsar/himgrow.git
cd himgrow

# Install dependencies
npm install

# Start development server
npm run dev
```

The app will be available at `http://localhost:5174`

---

## 📦 Build & Deployment

### Build for Production
```bash
npm run build
```

Output files will be in the `dist/` directory.

### Preview Production Build
```bash
npm run preview
```

### Deploy to Vercel
This project is auto-deployed on Vercel. Simply push to the `main` branch and Vercel will automatically build and deploy.

```bash
git push origin main
```

---

## 🧪 Code Quality

### Lint Check
```bash
npm run lint
```

---

## 📁 Project Structure

```
himgrow-technology/
├── src/
│   ├── App.jsx              # Main component with all sections
│   ├── App.css              # Component styles
│   ├── index.css            # Global styles & animations
│   ├── main.jsx             # React entry point
│   └── assets/              # Static assets
├── public/                  # Public files
├── index.html               # HTML template
├── vite.config.js           # Vite configuration
├── tailwind.config.js       # Tailwind CSS config
├── postcss.config.js        # PostCSS config
├── vercel.json              # Vercel deployment config
└── package.json             # Dependencies & scripts
```

---

## 🎯 Navigation

Quick links to site sections:
- **About** - Professional overview
- **Features** - Technical capabilities
- **Expertise** - Core competencies
- **Projects** - Featured work
- **Skills** - Technical toolkit
- **Contact** - Get in touch

---

## 📧 Contact & Connect

- **Email**: [vabhawsar@gmail.com](mailto:vabhawsar@gmail.com)
- **Phone**: [+91 9399614149](tel:9399614149)
- **GitHub**: [@anshubhawsar](https://github.com/anshubhawsar)
- **Location**: India 🇮🇳

---

## 🎨 Design Highlights

- **Color Scheme**: Rose, Pink, and Fuchsia gradients with dark mode support
- **Typography**: Bold, modern font stack with excellent readability
- **Animations**: Smooth transitions, reveal effects, and floating elements
- **Accessibility**: Semantic HTML, proper contrast ratios, keyboard navigation

---

## 📄 License

This project is licensed under the MIT License - see the LICENSE file for details.

---

## 🤝 Contributing

Contributions are welcome! Feel free to fork, improve, and submit pull requests.

---

## ✨ Credits

Built with ❤️ by **Anshu Bhawsar**

Evolving with purpose. 🚀

---

<div align="center">

**[⬆ Back to top](#-himgrow---evolving-the-digital-soul)**

</div>
